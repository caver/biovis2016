...
The set faces those tasks which are hard to address using the existing 1D, 2D, and 3D representations.
...
Therefore, we decided to propose a solution which will cover these requirements and will be simple enough and easy to use. 
...
First, we review the existing approaches to abstracted representation 1D or 2D representation of molecules.
...
For each aligned protein this algorithm computes the transformed positions of its atoms and the RMSD (root-mean-square deviation) number expressing the difference between the reference and the aligned protein.  
